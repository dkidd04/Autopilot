...
public class View extends AbstractBean {
    ...
    public JRootPane getRootPane() {
        if (rootPane == null) {
            ...
            rootPane.setContentPane( new JScrollPane() ) ;
        }
        return rootPane;
    }
    ...
    private void replaceContentPaneChild(JComponent oldChild, JComponent newChild, String constraint) {
        ...
        //
        // not sure...make all CENTER components have an implied JScrollPane wrapper.
        // this is done so that if the designer chose absolute layout, the
        // view isn't clipped when made too small.
        //
        if( constraint == BorderLayout.CENTER ) {
        	BorderLayout layout = (BorderLayout) contentPane.getLayout();
        	Component comp = layout.getLayoutComponent( constraint ) ;
        	if( comp == null ) {
        		comp = new JScrollPane() ;
        		contentPane.add( comp, constraint ) ;
        	}
        	JScrollPane sp = (JScrollPane) comp ;
        	sp.setViewportView( newChild ) ;
        	return;
        }
        if (oldChild != null) {
